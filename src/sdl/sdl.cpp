...
extern int switch_mode_g;
extern char new_game_path_g[];
...
inline size_t cbuf_write(cbuf_t *cbuf, uint8_t *src, size_t size) {
	size_t j;
	size_t k;
...
inline size_t cbuf_read(uint8_t *dst, cbuf_t *cbuf, size_t size) {
	if (size > cbuf->s)
		size = cbuf->s;
...
unsigned long pd_usecs(void) {
	...
	return (unsigned long) ((ts.tv_sec * 1000000) + (ts.tv_nsec / 1000000) );
#endif
}
...
struct filter {
	const char *name;
	void (*func)(bpp_t buf, unsigned int buf_pitch, unsigned int xsize,
			unsigned int ysize, unsigned int bpp);
};
...
struct scaling {
	const char *name;
	void (*func)(bpp_t dst, unsigned int dst_pitch, bpp_t src,
			unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
			unsigned int ysize, unsigned int yscale, unsigned int bpp);
};

static void (*scaling)(bpp_t dst, unsigned int dst_pitch, bpp_t src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale, unsigned int bpp);
...
static void do_screenshot(void) {
	...
	case 32:
		break;
	default:
		snprintf(msg, sizeof(msg), "Screenshots unsupported in %d bpp.",
				mdscr.bpp);
		pd_message(msg);
		return;
	}
	...
	stopped = 1;
	retry: snprintf(name, sizeof(name), "shot%06u.tga", n);
	...
	// Header
	{
		uint8_t tmp[(3 + 5)] = { 0x00, // length of the image ID field
	...
	{
		uint8_t tmp[2] = { 24, // always output 24 bits per pixel
				(1 << 5) // top-left origin
				};
...
static void rescale_32_1x1(uint32_t *dst, unsigned int dst_pitch, uint32_t *src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
...
static void rescale_32_any(uint32_t *dst, unsigned int dst_pitch, uint32_t *src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale) {
	unsigned int y;
...
static void rescale_24_1x1(uint24_t *dst, unsigned int dst_pitch, uint24_t *src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale) {
	unsigned int y;
...
static void rescale_24_any(uint24_t *dst, unsigned int dst_pitch, uint24_t *src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale) {
	unsigned int y;
...
static void rescale_16_1x1(uint16_t *dst, unsigned int dst_pitch, uint16_t *src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale) {
	unsigned int y;
...
static void rescale_16_any(uint16_t *dst, unsigned int dst_pitch, uint16_t *src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale) {
	unsigned int y;
...
static void rescale_8_1x1(uint8_t *dst, unsigned int dst_pitch, uint8_t *src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale) {
	unsigned int y;
...
static void rescale_8_any(uint8_t *dst, unsigned int dst_pitch, uint8_t *src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale) {
	unsigned int y;
...
static void rescale_1x1(bpp_t dst, unsigned int dst_pitch, bpp_t src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale, unsigned int bpp) {
	switch (bpp) {
	case 32:
		rescale_32_1x1(dst.u32, dst_pitch, src.u32, src_pitch, xsize, xscale,
				ysize, yscale);
		break;
	case 24:
		rescale_24_1x1(dst.u24, dst_pitch, src.u24, src_pitch, xsize, xscale,
				ysize, yscale);
		break;
	case 16:
	case 15:
		rescale_16_1x1(dst.u16, dst_pitch, src.u16, src_pitch, xsize, xscale,
				ysize, yscale);
		break;
	case 8:
		rescale_8_1x1(dst.u8, dst_pitch, src.u8, src_pitch, xsize, xscale,
				ysize, yscale);
		break;
	}
}

static void rescale_any(bpp_t dst, unsigned int dst_pitch, bpp_t src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale, unsigned int bpp) {
	if ((xscale == 1) && (yscale == 1)) {
		scaling = rescale_1x1;
		rescale_1x1(dst, dst_pitch, src, src_pitch, xsize, xscale, ysize,
				yscale, bpp);
		return;
	}
	switch (bpp) {
	case 32:
		rescale_32_any(dst.u32, dst_pitch, src.u32, src_pitch, xsize, xscale,
				ysize, yscale);
		break;
	case 24:
		rescale_24_any(dst.u24, dst_pitch, src.u24, src_pitch, xsize, xscale,
				ysize, yscale);
		break;
	case 16:
	case 15:
		rescale_16_any(dst.u16, dst_pitch, src.u16, src_pitch, xsize, xscale,
				ysize, yscale);
		break;
	case 8:
		rescale_8_any(dst.u8, dst_pitch, src.u8, src_pitch, xsize, xscale,
				ysize, yscale);
		break;
	}
}
...
static void rescale_hqx(bpp_t dst, unsigned int dst_pitch, bpp_t src,
		unsigned int src_pitch, unsigned int xsize, unsigned int xscale,
		unsigned int ysize, unsigned int yscale, unsigned int bpp) {
	if (xscale != yscale)
		goto skip;
	...
	case 32:
		switch (xscale) {
		case 2:
			hq2x_32_rb(src.u32, src_pitch, dst.u32, dst_pitch, xsize, ysize);
			return;
		case 3:
			hq3x_32_rb(src.u32, src_pitch, dst.u32, dst_pitch, xsize, ysize);
			return;
		case 4:
			hq4x_32_rb(src.u32, src_pitch, dst.u32, dst_pitch, xsize, ysize);
			return;
		}
		break;
	case 16:
	case 15:
		switch (xscale) {
		case 2:
			hq2x_16_rb(src.u16, src_pitch, dst.u16, dst_pitch, xsize, ysize);
			return;
		}
		break;
	}
	skip: rescale_any(dst, dst_pitch, src, src_pitch, xsize, xscale, ysize,
			yscale, bpp);
}
...
static void filter_blur_u32(uint32_t *buf, unsigned int buf_pitch,
	...
	for (y = 0; (y < ysize); ++y) {
		...
		for (x = 0; (x < xsize); ++x) {
			...
			tmp = (((((tmp & 0x00ff00ff) + (old & 0x00ff00ff)) >> 1)
					& 0x00ff00ff)
					| ((((tmp & 0xff00ff00) + (old & 0xff00ff00)) >> 1)
							& 0xff00ff00));
			old = buf[x];
			buf[x] = tmp;
		}
...
static void filter_blur_u16(uint16_t *buf, unsigned int buf_pitch,
	...
	for (y = 0; (y < ysize); ++y) {
		...
		for (x = 0; (x < xsize); ++x) {
			...
			tmp = (((((tmp & 0xf81f) + (old & 0xf81f)) >> 1) & 0xf81f)
					| ((((tmp & 0x07e0) + (old & 0x07e0)) >> 1) & 0x07e0));
			old = buf[x];
			buf[x] = tmp;
		}
#endif
...
static void filter_blur_u15(uint16_t *buf, unsigned int buf_pitch,
	...
	for (y = 0; (y < ysize); ++y) {
		...
		for (x = 0; (x < xsize); ++x) {
			...
			tmp = (((((tmp & 0x7c1f) + (old & 0x7c1f)) >> 1) & 0x7c1f)
					| ((((tmp & 0x03e0) + (old & 0x03e0)) >> 1) & 0x03e0));
			old = buf[x];
			buf[x] = tmp;
		}
#endif
...
static void filter_blur(bpp_t buf, unsigned int buf_pitch, unsigned int xsize,
		unsigned int ysize, unsigned int bpp) {
	switch (bpp) {
	case 32:
...
static void filter_scanline_frame(bpp_t buf, unsigned int buf_pitch,
		unsigned int xsize, unsigned int ysize, unsigned int bpp,
		unsigned int frame) {
	unsigned int y;
...
// No-op filter.
static void filter_off(bpp_t buf, unsigned int buf_pitch, unsigned int xsize,
		unsigned int ysize, unsigned int bpp) {
	(void) buf;
	(void) buf_pitch;
	(void) xsize;
...
// The first four filters must match ctv_names in rc.cpp.
		{ "off", filter_off }, { "blur", filter_blur }, { "scanline",
				filter_scanline }, { "interlace", filter_interlace }, { NULL,
				NULL } };
...
int pd_graphics_init(int want_sound, int want_pal, int hz) {
	...
	if (SDL_Init(
			want_sound ?
					(SDL_INIT_VIDEO | SDL_INIT_AUDIO) : (SDL_INIT_VIDEO))) {
...
//				(SDL_HWPALETTE | SDL_HWSURFACE | SDL_DOUBLEBUF
			...
			//	(SDL_HWPALETTE | SDL_HWSURFACE | SDL_DOUBLEBUF | SDL_ASYNCBLIT
						| (fullscreen ? SDL_FULLSCREEN : 0)));
	}
...
#endif
	// Copy/rescale output.
	scaling(dst, dst_pitch, src, src_pitch, xsize, x_scale, ysize, y_scale,
			mdscr.bpp);
#ifdef WITH_CTV
...
int pd_sound_init(long &freq, unsigned int &samples) {
	...
	fprintf(stderr, "sound: %uHz, %d samples, buffer: %u bytes  video_hz %d\n",
	sound.rate, spec.samples, (unsigned int)sound.cbuf.size, video_hz);
...
enum kb_input {
	KB_INPUT_ABORTED, KB_INPUT_ENTERED, KB_INPUT_CONSUMED, KB_INPUT_IGNORED
};
...
#define HISTORY_LEN 32
	...
	if (((ks->unicode & 0xff80) == 0)
		...
		memmove(&input->buf[input->pos], &input->buf[(input->pos + 1)], tail);
		return KB_INPUT_CONSUMED;
...
extern const char *rombrowser_find(const char *srch);
static const char *stop_get_user_input() {
	SDL_Event event;
	...
	//	fprintf(stderr,"stop_get_user_input: wait for key press ...\n");

	  switch (event.type) {
		case SDL_KEYDOWN:
		//	fprintf(stderr,"stop_get_user_input: key pressed.\n");
			switch (kb_input(&input, &event.key.keysym))
			{
		      case KB_INPUT_ENTERED:
			       pd_message(input.buf);
			       fprintf(stderr,"entered: '%s'\n", input.buf);
			       virtualkeyboard_hide();
			       SDL_PauseAudio(0);
			       return rombrowser_find(input.buf);
			  ...
			  case KB_INPUT_CONSUMED:
			       fprintf(stderr,"consumed:'%s'", input.buf);
			       continue;
		     }
			 break;
	  }
...
// in, and swap on the fly like DirectX DGen. [PKH]
			...
			{
				strncpy( new_game_path_g, stop_get_user_input() , 127);
				fprintf(stderr,"user search rom is '%s' \n", new_game_path_g);

				if( strcmp( new_game_path_g,"") != 0) {
				   pd_message(new_game_path_g);
				   load_new_game_g = 2;
				} else {
					pd_message("no matching rom found.");
					fprintf(stderr,"no match, returning back to game ...\n");
				}
			}

			else if (ksym == dgen_quit) {
				switch_mode_g = 1;
				pd_message("PAL/NTSC toggle with reset");
			}
			...
			extern void shutdown_quit(md& megad);
			shutdown_quit(megad);
			return 0;
			default:
			break;
